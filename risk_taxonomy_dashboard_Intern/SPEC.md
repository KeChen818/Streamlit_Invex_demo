...
The embedding layer uses LangChain `OpenAIEmbeddings` with `text-embedding-3-large` and a FAISS vector store. If dependencies or `OPENAI_API_KEY` are unavailable, the app must show an explicit setup/runtime error and skip theme generation for that page load.
...
The intern MVP uses OpenAI embeddings and a FAISS vector store for similarity search. Future deployments may explicitly replace or extend this layer with:
...
- Show an explicit AI error if GPT is unavailable.
...
- If `OPENAI_API_KEY` is absent or a call fails, show an explicit AI error and do not substitute local AI analysis.
...
- `risk_dashboard/analysis.py`: deterministic analytics used by views and prompts.
...
- `risk_dashboard/themes.py`: semantic payloads, OpenAI/FAISS embeddings, hybrid similarity relationships, theme clustering, Business Division/GCRS count summaries, and review fields.
...
- AI Chatbot filters the associated-risk table by returned `Group_ID`s.
