...
def tokenize(text: str) -> list[str]:
    """Tokenize text for keyword signatures and similarity sub-scores."""
...
def build_openai_faiss_embeddings(data: pd.DataFrame, payloads: list[str]) -> np.ndarray:
    """Embed weighted risk payloads with OpenAI and load them into FAISS."""
    if not os.getenv("OPENAI_API_KEY"):
        raise RuntimeError("OPENAI_API_KEY is required for AI Risk Theme Classification embeddings.")

    documents = build_langchain_documents(data, payloads)
    if not documents:
        raise ValueError("No risk documents were available for embedding.")
    ...
    vectors = [vector_store.index.reconstruct(index) for index in range(vector_store.index.ntotal)]
    if len(vectors) != len(documents):
        raise RuntimeError("FAISS vector count did not match the embedded risk document count.")
...
def build_embedding_layer(data: pd.DataFrame, payloads: list[str]) -> np.ndarray:
    """Build the OpenAI/FAISS embedding matrix used by theme classification."""
    return build_openai_faiss_embeddings(data, payloads)


def calibrated_similarity(cosine_similarity: float) -> float:
    """Map cosine similarity into the business threshold scale."""
...
def cluster_theme_signature(data: pd.DataFrame, cluster: list[str]) -> str:
    ...
    return normalize_signature(default_signature) or "|".join(cluster)
...
def build_theme_classification(
    ...
    """Run the AI theme classification pipeline."""
