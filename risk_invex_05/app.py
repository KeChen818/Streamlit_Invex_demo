...
def metric_values(data: pd.DataFrame, filter_note: str) -> list[tuple[str, str, str, str]]:
    high_risks = int((data["Impact"] == "High").sum())
    high_likelihood = int((data["Likelihood"] == "High").sum())
...
def render_heatmap_chart(
    ...
    heatmap_points, heatmap_max = build_heatmap_points(data)
...
def render_home_page(data: pd.DataFrame, filter_note: str) -> None:
    ...
    with left:
        ...
        if isinstance(clicked_cell, dict):
            ...
            if impact_index in range(len(RISK_LEVELS)) and likelihood_index in range(len(RISK_LEVELS)):
                selected_cell = (RISK_LEVELS[impact_index], RISK_LEVELS[likelihood_index])
                st.session_state.home_heatmap_cell = selected_cell

        table_data = data
        if selected_cell:
            impact, likelihood = selected_cell
            table_data = data[(data["Impact"] == impact) & (data["Likelihood"] == likelihood)]
    ...
    with right:
        render_chat_panel(data, filter_note, "home")
        st.markdown(
            f"""
    ...
    with right:
        ...
        st.markdown(
            ...
            <div class="insight-card">
              ...
              <div class="alert-row"><span>High impact</span><b class="risk-high">{int((data['Impact'] == 'High').sum())}</b></div>
              <div class="alert-row"><span>High likelihood</span><b class="risk-med">{int((data['Likelihood'] == 'High').sum())}</b></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def render_issues_page(data: pd.DataFrame, filter_note: str) -> None:
    ...
    issues = data[
        (data["DQ Score"] < 90)
        | ((data["Impact"] == "High") & data["Change"].isin(["New", "Increased"]))
        | ((data["Impact"] == "High") & (data["Likelihood"] == "High"))
    ].copy()
