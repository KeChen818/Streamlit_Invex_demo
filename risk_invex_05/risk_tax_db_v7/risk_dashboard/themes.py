...
def build_semantic_payload(row: pd.Series) -> str:
    """Build the standardized semantic payload used for theme similarity."""
    return f"""
Risk ID: {field_text(row, "Group_ID")}

Risk Name:
{field_text(row, "Risk_Title")}
...
Taxonomy:
{field_text(row, "Taxonomy_L1")} / {field_text(row, "Taxonomy_L2")}
...
def build_theme_classification(
    ...
    clusters = cluster_risks(scoped["Group_ID"].astype(str).tolist(), relationships, cluster_threshold)
